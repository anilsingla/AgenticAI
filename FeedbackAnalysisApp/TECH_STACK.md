...
- **LLM & Orchestration**: OpenAI GPT-4o-mini via AutoGen
- **Vector Database**: ChromaDB (semantic search and RAG)
- **Data Persistence**: SQLite + SQLAlchemy ORM
- **Frontend**: Streamlit (8-page multipage app)
- **Containerization**: Docker + Docker Compose
- **Runtime**: Python 3.14

## Feature Set

- Multi-agent feedback processing pipeline (6 agents)
- RAG-enhanced classification and analysis
- Duplicate detection via vector similarity
- Quality assurance automation
- Real-time dashboard with manual overrides
- Comprehensive audit logging
- Deterministic fallback (no API key required)

## Architecture

Data flows through 6 processing stages:
1. **Ingestion** - Load feedback from CSV
2. **Classification** - Categorize into Bug/Feature/Praise/Complaint/Spam
3. **Bug Analysis** - Extract technical details and match known issues
4. **Feature Extraction** - Evaluate impact and roadmap alignment
5. **Ticket Generation** - Create standardized tickets with duplicate detection
6. **Quality Review** - Score and revise low-quality outputs

All data is persisted in SQLite, searchable vectors in ChromaDB, and outputs exported as CSV/JSON for downstream integration.
System Objectives
