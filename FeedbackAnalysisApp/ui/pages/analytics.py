...
def render():
    ...
    col1.metric("Latest Accuracy", f"{float(latest.get('accuracy', 0.0)):.0%}")
    col2.metric("Latest Avg Confidence", f"{float(latest.get('avg_confidence', 0.0)):.0%}")
    col3.metric("Latest Processing Time", f"{float(latest.get('processing_time_seconds', 0.0)):.1f}s")
    ...
    if len(df_metrics) > 1:
        ...
        if "accuracy" in df_metrics.columns:
            ...
            st.line_chart(df_metrics.set_index("run_id")["accuracy"])
