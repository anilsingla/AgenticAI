...
from agents.heuristics import build_ticket
...
TICKET_PROMPT = """You are a project manager creating a structured ticket from user feedback for SignalDesk.
...
def ticket_creator_agent(state: PipelineState) -> dict:
    ...
    actionable_count = sum(1 for item in items if item.get("category") in ACTIONABLE_CATEGORIES)
    logger.info(
        "[FLOW] Ticket Creator started: %d actionable items, mode=%s.",
        actionable_count,
    ...
    for item in items:
        ...
        try:
            # â”€â”€ RAG: check for duplicate/similar feedback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            # â”€â”€ Build analysis context â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            analysis_parts = []
            ...
            if use_llm:
                ticket = run_json_agent(
                    "You are a project manager creating structured engineering tickets. Return valid JSON only.",
                    prompt,
                    agent_name="ticket_creator_agent",
                    temperature=0.0,
                )
            else:
                ticket = build_ticket(item, item["similar_tickets"])
            item["ticket"] = ticket
            ...
            if ticket.get("is_duplicate"):
                ...
                logger.info("%s â†’ DUPLICATE of %s", item["source_id"], item["duplicate_of"])
            else:
                ...
                logger.info("%s â†’ ticket: %s", item["source_id"], ticket["title"])
    ...
    created_count = sum(1 for item in items if item.get("ticket", {}).get("title"))
    duplicate_count = sum(1 for item in items if item.get("ticket", {}).get("is_duplicate"))
    logger.info(
        "[FLOW] Ticket Creator completed: %d tickets generated, %d duplicates flagged (%d errors).",
        created_count,
        duplicate_count,
        len(errors),
    )
